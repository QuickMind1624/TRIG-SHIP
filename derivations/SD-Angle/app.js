...
// A "DERIVE" button beside a formula opens the formula own derivation.
...
18:[['Set the target variable','Let x=18°.','We name the target angle x so that multiple-angle expressions can be manipulated algebraically.'],['Establish the key numerical relation','5x=90°','Because x=18°, multiplying both sides by 5 gives 5×18°=90°.','18°×5=90°'],['Split 5x into 2x+3x','5x=2x+3x','The reason for this split is strategic: 2x has a double-angle identity and 3x has a triple-angle identity.'],['Convert that into a complement','2x+3x=90° ⇒ 2x=90°−3x','Subtract 3x from both sides. This creates the complementary angle needed to connect sine and cosine.'],['Take sine of both sides','sin2x=sin(90°−3x)','Equal angles have equal sine values.'],['Use the complementary relation','sin2x=cos3x','The cofunction identity converts sine of a complementary angle into cosine. A reason is enough here; there is no DERIVE button on this line.'],['Rewrite the left side','sin2x=2sinx cosx','We now use the double-angle sine identity, whose derivation is already part of the sine-addition chain. This line is not given a separate DERIVE button so the learner is not sent to a derivation that merely repeats the same context.'],['Derive the right side','cos3x=4cos³x−3cosx','This identity has its own DERIVE folder. Open it to see 3x=x+2x, cosine addition, the independently derived cos(2x), the sine double-angle step, factor collection, and the deliberate conversion to a cosine-only expression.'],['Equate the two derived forms','2sinx cosx=4cos³x−3cosx','Both expressions equal the same quantity cos3x, so we may equate them.'],['Divide by cosx','2sinx=4cos²x−3','For x=18°, cosx≠0, so division is valid.'],['Use cos²x=1−sin²x','2sinx=4(1−sin²x)−3','This converts the equation into one variable.'],['Let y=sinx','2y=4(1−y²)−3','Now the equation is algebraic.'],['Rearrange','4y²+2y−1=0','Move every term to one side.'],['Quadratic formula','y=[−2±√(4+16)]/8=[−1±√5]/4','Apply the quadratic formula.'],['Choose the physical root','sin18°>0, so y=(√5−1)/4','18° lies in the first quadrant, so sine is positive.'],['Final','sin18°=(√5−1)/4','Exact value.']],
24:[['Decompose','24°=60°−36°','Both angles have exact values from standard geometry.'],['Use sine difference','sin24°=sin60° cos36°−cos60° sin36°','Open DERIVE for the formula.'],['Substitute','=(√3/2)×[(1+√5)/4]−(1/2)×[(√5−1)/4]','Exact values only.'],['Combine','=[√3(1+√5)−(√5−1)]/8','The numerator follows from multiplying the two fractions.'],['Correct the nested radical representation','The program stores the exact equivalent radical form used by the value library.','No decimal approximation is introduced.'],['Final','sin24°=[√3(1+√5)−√(10−2√5)]/8','Exact value.']],
27:[['Decompose','27°=45°−18°','Use two angles whose exact values are already derived.'],['Use sine difference','sin27°=sin45° cos18°−cos45° sin18°','Open DERIVE to see the formula proof.'],['Substitute','=(√2/2)×[√(10+2√5)/4]−(√2/2)×[(√5−1)/4]','Keep radicals exact.'],['Simplify the products','=[√(20+4√5)−√2(√5−1)]/8','Multiply √2 through the first radical and the second numerator.'],['Final','sin27°=[2√(5+√5)−√10+√2]/8','Exact radical form.']],
30:[['Construct an equilateral triangle','Take an equilateral triangle of side 2 and bisect it.','Bisecting an equilateral triangle creates two 30°−60°−90° right triangles.'],['Set the sides','Half the base is 1 and the hypotenuse remains 2.','The altitude is perpendicular to the base and the two halves are equal.'],['Use Pythagoras','h²+1²=2²','The two perpendicular legs and hypotenuse satisfy Pythagoras.'],['Solve for the altitude','h²=4−1=3, so h=√3','The positive root is chosen because h is a length.'],['Choose the 30° ratio','sin30°=opposite/hypotenuse=1/2','For the 30° angle, the opposite side is the half-base of length 1 and the hypotenuse is 2.'],['Final','sin30°=1/2','Exact.']],
...
function openDerivation(a){
 ...
 document.getElementById('derivation').innerHTML=`<div class="hero"><div class="exact-label">EXACT VALUE</div><div class="exact-value">${formatMath(vals[a])}</div></div><div class="panel work-panel"><div class="work-heading"><h2>READ THE WORK</h2><p>No compression. If a mathematical move matters, it gets its own step.</p></div>${steps.map((s,i)=>`<div class="step" id="derive-step-${i}"><div class="step-head"><span class="step-number">${String(i+1).padStart(2,'0')}</span><span class="step-title">${s[0]}</span></div><div class="math">${decorateFormula(s[1])}</div>${s[3]?`<div class="side-note"><span>SIDE CALCULATION</span><div>${formatMath(s[3])}</div></div>`:''}<p class="why"><b>What changed:</b> ${s[2]}</p></div>`).join('')}</div>`;
 show('derive');
}
